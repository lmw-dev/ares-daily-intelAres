...
def main():
    ...
    gcs_full_path = f"gs://{settings.gcs_bucket}/{gcs_dir}/scan.md"
    ...
    # 6. 保存报告（双写：本地 + 谷歌云存储）
    try:
        save_to_local(scan_date, markdown_report, report_json, scan_results)
    except Exception as e:
        logger.error(f"Failed to save reports locally: {e}")

    try:
        upload_to_gcs(scan_date, markdown_report, report_json, scan_results)
    except Exception as e:
        logger.error(f"Failed to upload reports to GCS: {e}")

    # 7. 推送消息至 Slack
    try:
        push_to_slack(report_json, scan_date)
    except Exception as e:
